...
class Tree:
    ...
    def build(self, ws):
        self.root = self._build_recursive(ws, 1)
        return self.root

    def _build_recursive(self, ws, n):
        if n <= 0:
            return None
        elem = ws.pop(0)
        if type(elem) == str:
            node = Node(elem, None, None)
            node.right_sibling = self._build_recursive(ws, n - 1)
            return node
        else:
            node = Node(elem[0], None, None)
            no_productions = len(self.grammar.non_terminal_productions(elem[0])[elem[1] - 1])
            node.child = self._build_recursive(ws, no_productions)
            node.right_sibling = self._build_recursive(ws, n - 1)
            return node
